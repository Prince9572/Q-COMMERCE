...
const Navbar = () => {
    const user: any = {name: "Admin", email: "admin@example.com", isAdmin: true}
    ...
    const [searyQuery, setSearchQuery] = useState("")
    ...
    const handleSearch = (e: React.SubmitEvent) => {
        e.preventDefault();
        if(searyQuery.trim()){
            navigate(`/search?q=${encodeURIComponent(searyQuery.trim())}`)
            setSearchQuery("")
        }
    }
                        ...
                        {userMenuOpen && (
                            <>
                                <div className="fixed inset-0 z-40" onClick={()=>
                                    setUserMenuOpen(false)}/>
                                    <div className="absolute right-0 mt-2.5 mt-2.5 w-56 bg-white rounded-xl shadow-lg border border-app-border py-2 z-50 animate-fade-in">
                                        {user && (
